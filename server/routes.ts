...
import { insertSamagamSchema, insertRecordedSamagamSchema, insertLocationSchema, users } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/samagams", async (req, res) => {
    ...
    try {
      const { samagams, total } = await storage.getPaginatedSamagams(limit, offset);
      res.json({
        samagams,
        pagination: {
          page,
  ...
  app.post("/api/samagams", authenticate, requireAdmin, upload.single('image'), handleMulterError, async (req: Request, res: Response) => {
    try {
      ...
      if (req.file) {
        try {
          ...
          return res.status(500).json({ 
            message: imageError instanceof Error ? imageError.message : "Error uploading image"
          });
        }
      }
  ...
  app.patch("/api/samagams/:id", authenticate, requireAdmin, upload.single('image'), handleMulterError, async (req: Request, res: Response) => {
    try {
      // The form data is in req.body as JSON string
      let formData;
      try {
  ...
  app.delete("/api/samagams/:id", authenticate, requireAdmin, async (req, res) => {
    const deleted = await storage.deleteSamagam(parseInt(req.params.id));
    if (!deleted) return res.status(404).json({ message: "Samagam not found" });
    res.status(204).send();
  });
  ...
  app.get("/api/recorded-samagams", async (req, res) => {
    ...
    try {
      const { recordedSamagams, total } = await storage.getPaginatedRecordedSamagams(limit, offset);
      res.json({
        recordedSamagams,
        pagination: {
          page,
  ...
  app.post("/api/recorded-samagams", authenticate, requireAdmin, async (req, res) => {
    try {
      const validatedData = insertRecordedSamagamSchema.parse({
        ...req.body,
        ...(req?.body?.date ? {date: new Date(req.body.date)} : {})
      });
      const recordedSamagam = await storage.createRecordedSamagam(validatedData, req.user!.id);
  ...
  app.patch("/api/recorded-samagams/:id", authenticate, requireAdmin, async (req, res) => {
    try {
      const validatedData = insertRecordedSamagamSchema.parse({
        ...req.body,
        ...(req?.body?.date ? {date: new Date(req.body.date)} : {})
      ...
      if (!recordedSamagam) return res.status(404).json({ message: "Recorded Samagam not found" });
  ...
  app.delete("/api/recorded-samagams/:id", authenticate, requireAdmin, async (req, res) => {
    const deleted = await storage.deleteRecordedSamagam(parseInt(req.params.id));
    if (!deleted) return res.status(404).json({ message: "Recorded Samagam not found" });
    res.status(204).send();
  });
  ...
  app.post("/api/locations", authenticate, requireAdmin, async (req, res) => {
    try {
      ...
      const location = await storage.createLocation(validatedData, req.user!.id);
  ...
  app.patch("/api/locations/:id", authenticate, requireAdmin, async (req, res) => {
    try {
      const validatedData = insertLocationSchema.parse(req.body);
  ...
  app.delete("/api/locations/:id", authenticate, requireAdmin, async (req, res) => {
  ...
  app.get("/api/fcm-tokens/check/:token", async (req: Request, res: Response) => {
    try {
      const { token } = req.params;
      if (!token) {
        return res.status(400).json({ message: "FCM token is required" });
